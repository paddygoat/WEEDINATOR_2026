...
// int RH_LIMITER = -50000;
// int LH_LIMITER = 50000;
...
void TaskControlRobotMan(void *pvParameters)
{
  ...
  { 
    ...
    if(Man_Auto == true) // ie Manual mode.
    {
      ...
      // Create stepped Proportional (P) for steering:
      int steeringDifferential = abs(y - actSteerAngle);

      if(steeringDifferential > 4000)
      {
        myPWM = 250;
      }
      if(steeringDifferential <= 4000)
      ...
      if(steeringDifferential <= 3000)
      {
        myPWM = 150;
      }
      if(steeringDifferential <= 2000)
      {
        myPWM = 100;
      }
      ...
      // Steering dead zone:
      if((y < (actSteerAngle +32)) && (y > (actSteerAngle -32)))
      {
        myPWM = 0;
      }
    } //     if(Man_Auto == true) // ie Manual mode.
    ...
    if((actSteerAngle < RH_LIMITER ) && (motor_steering_DIR == false))  // Turn RIGHT.
    {
      //Serial.print("ARRIVED!!");
      myPWM = 0;
    }

    if((actSteerAngle > LH_LIMITER ) && (motor_steering_DIR == true))  // Turn LEFT.
    {
      myPWM = 0;
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(PWMsmoothingThrotA > encoderThrotAVal)
    {
      ...
      if(throtA_differential <= 800)
      {
        ThrotASpeed= 150;
      }
      if(throtA_differential <= 400)
      {
        ThrotASpeed= 130;
      }
    ...
    // Throttle dead zone:
      if((throtA_differential >= -160) && (throtA_differential <= 160))
      {
        ThrotASpeed= 0;
      }
...
////////////////////////////////////////////////////////////////////////////////
    // ch2 min = 229    
    z = (ch2_data - 229) * 0.8;                   // desired position (z).
    // encoderThrotBVal= encThrotB.getTicks();    // actual position.
    
    // Create PID for Throttle B (Stepped Proportional):
    int throtBDifferential = abs(z - encoderThrotBVal); 
    int ThrotBSpeed = 0;

    // 1. Determine Direction
    if(z > encoderThrotBVal)
    {
      motor_throtB_DIR = false;                   // throttle opens.
    }
    if(z < encoderThrotBVal)
    {
      motor_throtB_DIR = true;                    // throttle closes.
    }

    // 2. Set Stepped Proportional Speed based on error
    if(throtBDifferential > 800)
    {
      ThrotBSpeed = 250;
    }
    if(throtBDifferential <= 800)
    ...
    if(throtBDifferential <= 500)
    {
      ThrotBSpeed = 150;
    }
    if(throtBDifferential <= 250)
    {
      ThrotBSpeed = 100;
    }

    // 3. Apply Dead Zone (prevents motor jitter at the target)
    if((z < (encoderThrotBVal + 60)) && (z > (encoderThrotBVal - 60)))
    {
      ThrotBSpeed = 0;
    }

    // 4. Write to Motor Hardware
