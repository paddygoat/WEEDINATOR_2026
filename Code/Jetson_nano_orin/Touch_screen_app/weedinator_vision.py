...
def get_memory_utilization() -> dict:
    """Reads RAM usage directly from /proc/meminfo without external dependencies."""
    mem_info = {}
    path = "/proc/meminfo"
        ...
        try:
            with open(path, "r") as f:
                for line in f:
                    ...
                    if len(parts) == 2:
                        key = parts[0].strip()
                        # Extract the numerical value (in kilobytes)
            ...
            if total_kb > 0:
                ...
                return {
                    ...
                    "used_gb": round(used_kb / 1024**2, 2), # Convert KB to GB
                    "total_gb": round(total_kb / 1024**2, 2)
                }
        except Exception as e:
            print(f"[Vision Error] Failed to read memory info: {e}")

    # Fallback if it fails
    return {"percent": 0.0, "used_gb": 0.0, "total_gb": 0.0}

def get_gpu_utilization() -> float:
    """Reads the Orin Nano GPU load percentage directly from sysfs."""
    ...
    if os.path.exists(path):
        try:
            with open(path, "r") as f:
                val = f.read().strip()
                if val:
                    # NVIDIA raw load / 10 gives the percentage
                    return int(val) / 1000.0 
        except Exception:
            pass # Fail silently to keep the console clean
    ...
    while True:
        ...
        if camera_start_state == True:
            ...
            if RECORD_CPU and (current_epoch_time - last_cpu_read_time > RECORD_CPU_TIME_INTERVAL):
                ...
                mem_data = get_memory_utilization()
                # print(f"RAM Used: {mem_data['used_gb']} GB ({mem_data['percent']}%)")
...
#################################################################################################################
        ...
        elif camera_start_state == False:
            time.sleep(0.5) # Prevent CPU race
            # print("Camera frame processing loop turned off")
